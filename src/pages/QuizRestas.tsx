...
const generateProblem = (): Problem => {
  // Para restas, asegurarnos de que el resultado sea positivo
  const a = randomInt(200, 999)
  const b = randomInt(100, a - 1) // b debe ser menor que a
  const correct = a - b

  const distractors = new Set<number>()
  while (distractors.size < 2) {
    const delta = randomInt(-150, 150)
...
export default function QuizRestas() {
  ...
  return (
    <main className={['quiz-app', isOverlayOpen ? 'overlay-open' : ''].filter(Boolean).join(' ')}>
      <header className="toolbar">
        <div>
          <h1>Sum Quiz - Restas ➖</h1>
          <p>Practica restas rápidas con tres plantillas distintas.</p>
        </div>
        <div className="stats">
        ...
        >
          {pages[currentPage]?.map((problem, index) => {
            ...
            return (
              ...
              >
                <span className="question-label">Resta {index + 1}</span>
